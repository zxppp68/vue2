...
v-if：通过元素上树与下树进行操作 需要操作dom，性能差
面试题:开发项目的时候，优化手段有哪些?
1:v-show|v-if
2:按需加载
...
query参数：路由不需要占位，写法类似于ajax当中query参数,里面是数组键值对例如id：666
可以query.id获取数据
路由传递参数先关面试题
     1:路由传递参数（对象写法）path是否可以结合params参数一起使用?
     不可以：不能这样书写，程序会崩掉，必须要有name属性，有了就可以
     现在可以这样 但是控制台提示不好

     2:如何指定params参数可传可不传? 
     配置路由时已经占位了，跳转时不传递参数路径会出现问题
        占位时在后面加一个? 就可以可以传可以不传

     3:params参数可以传递也可以不传递，但是如果传递是空串，如何解决？
    空串路径会出问题，可以params:{keyword:''||undefined}，保证路径正确
