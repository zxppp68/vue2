...
2:组件实例的$nextTick方法。
在下次DOM更新, 循环结束之后,执行延迟回调。在 修改数据之后 立即使用这个方法，获取更新后的DOM
、、、、监听异步数据遍历完了之后用nextTick

、、、、this.$ref.refName来获取元素，效果和querySelector一致
、、、、开发步骤：
静态页面+静态组件拆分出来、
写api，发请求、
vuex得到数据三连环state action mutation、
组件获取仓库数据 this.$store.dispatch()、computed,...mapstate
组件动态展示数据 v-for遍历、插值语法{{}}、组件通信等
...
、、、在内部发，无法创造两个floor组件派发action应该是在父组件的组件挂载完毕生命周期函数中书写，因为父组件需要通知Vuex发请求，父组件
