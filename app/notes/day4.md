...
、、、、、跳转之后TypeNav又会生成一个新的实例，因此又有新的生命周期
、、、、过渡动画前提是组件|元素有v-if 或者v-show 主要分为进入和结束阶段  并在外面用transition包裹,复习一下过渡动画
、、、、overflow:hidden;以及css权重问题
...
、、、、、最终解决方案：在App.
、、、、、因为有this指向，组件身上才有$store，这里的this应该指向window，所以不能在main.js上
...
、、、、合并query和params参数,if判断里对空对象,应该是true的说法
...
、、、vuex三连击再复习一下，以及原理,检查一下开启命名空间后怎么从仓库里调用数据
...
第四步：开始mock模拟数据，在mock文件夹中创建一个server.js文件
、、、、、注意：在server.js文件当中对于banner.json||floor.json的数据没有暴露，但是可以在server模块中使用。
、、、、、对于webpack当中一些模块：图片、json，不需要对外暴露，默认对外暴露。
...
、、、注意swiper实例创建必须要在结构完整之后---指的是真实dom出现才行，所以不能在mounted里面写，这时候有dispatch异步语句请求数据，导致v-for遍历的时候需要等待，所以组件身上可能还没数据，结构不完整
